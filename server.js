...
const rooms = {
    'room1': { players: {}, state: 'waiting', turn: null, turnTimer: null, scores: {} },
    'room2': { players: {}, state: 'waiting', turn: null, turnTimer: null, scores: {} },
    'room3': { players: {}, state: 'waiting', turn: null, turnTimer: null, scores: {} },
};
...
io.on('connection', (socket) => {
    ...
    socket.on('joinRoom', (roomId) => {
        ...
        // Надсилаємо оновлення гравців всім у кімнаті
        io.to(roomId).emit('playerUpdate', room.players); // Перенесено сюди

        if (Object.keys(room.players).length === 2) {
            // Ініціалізуємо рахунок ТІЛЬКИ коли другий гравець зайшов
            room.scores = {};
            for (const socketId in room.players) { room.scores[socketId] = 0; }
            startGame(roomId); // Запускаємо генерацію полів
        }

        io.emit('roomList', getRoomStatus()); // Оновлюємо статус кімнат для всіх у лобі
    });
    ...
    socket.on('makeMove', (coords) => {
        ...
        if (!opponentId) return; // Опонент міг вийти
    ...
    socket.on('disconnect', () => {
        ...
        if (currentRoom && rooms[currentRoom]) {
            ...
            const playerInfo = room.players[socket.id]; // Зберігаємо інфо про гравця, що вийшов
            delete room.players[socket.id]; // Видаляємо гравця

            const remainingPlayers = Object.keys(room.players);

            // Якщо гра тривала (playing) і залишився 1 гравець
            if (oldState === 'playing' && remainingPlayers.length === 1) {
                 const winnerId = remainingPlayers[0];
                 // Перевіряємо, чи існує room.scores перед додаванням очка
                 if (!room.scores) room.scores = {};
                 if (!room.scores[winnerId]) room.scores[winnerId] = 0;
                 room.scores[winnerId]++;
                 // Передаємо інфо про гравців, які були в кімнаті до виходу
                 const finalPlayersInfo = {...room.players};
                 if(playerInfo) finalPlayersInfo[socket.id] = playerInfo; // Додаємо інфо про гравця, що вийшов

                 io.to(winnerId).emit('gameOver', {
                     winner: room.players[winnerId].username,
                     disconnected: true,
                     scores: room.scores,
                     players: finalPlayersInfo // Надсилаємо повну інфу про гравців
                 });
                 room.state = 'post-game';
                 room.players[winnerId].ready = false;
            }
            // Якщо гравець вийшов зі стану очікування (waiting) і залишився 1 гравець
            else if (oldState === 'waiting' && remainingPlayers.length === 1) {
                // Повідомляємо гравця, що залишився
                 io.to(remainingPlayers[0]).emit('playerLeft');
                 // Не скидаємо кімнату повністю, просто чекаємо нового гравця
                 room.state = 'waiting'; // Залишаємо в очікуванні
            }
            // Якщо кімната стала порожньою (чи гравець вийшов з post-game)
            else if (remainingPlayers.length < 1) {
                resetRoom(currentRoom);
            }
            // Якщо залишився 1 гравець після post-game, йому треба вийти
            else if (oldState === 'post-game' && remainingPlayers.length === 1){
                 io.to(remainingPlayers[0]).emit('playerLeft');
            }

            io.emit('roomList', getRoomStatus()); // Оновлюємо статус кімнат
        }

        if (userWasActive) {
            broadcastOnlineCount(); // Оновлюємо лічильник онлайн
        }
    });
});
...
function startGame(roomId) {
    const room = rooms[roomId];
    if (!room || Object.keys(room.players).length < 2) return; // Перевірка на двох гравців
    // НЕ встановлюємо state тут, це робиться в 'playerReady'
    ...
    room.turnTimer = setTimeout(() => {
        ...
        setTimeout(() => {
            if (rooms[roomId] && rooms[roomId].state === 'playing') {
                ...
                startTurnTimer(roomId); // Рекурсивно для наступного
            }
        }, 2000); // 2 сек на читання
...
function resetRoom(roomId) {
    if (rooms[roomId]) {
        ...
        // Повністю скидаємо кімнату до початкового стану
        rooms[roomId] = { players: {}, state: 'waiting', turn: null, turnTimer: null, scores: {} };
        console.log(`Room ${roomId} has been reset.`);
        io.emit('roomList', getRoomStatus()); // Оновлюємо лобі
    }
}
...
function generateRandomBoard() {
    ...
    let ships = [];
    let attempts = 0; // Додаємо лічильник спроб для безпеки
    for (const size of SHIP_CONFIG) {
        let placed = false;
        let currentAttempts = 0;
        while (!placed && currentAttempts < 100) { // Обмеження на 100 спроб на корабель
            const o = Math.random() < 0.5; // true=hor, false=ver
            ...
            if (canPlaceShip(board, x, y, size, o)) {
                ...
                for (let i = 0; i < size; i++) {
                    const cX = o ? x + i : x;
                    const cY = o ? y : y + i;
                    board[cY][cX] = 1; // Позначаємо як корабель
                    shipPos.push({ x: cX, y: cY, hit: false });
                }
                ships.push({ size, positions: shipPos });
                markSurroundings(board, shipPos); // Позначаємо зону навколо
                placed = true;
            }
            currentAttempts++;
            attempts++;
         ...
         if (currentAttempts >= 100) {
            ...
            // Можна додати логіку перезапуску генерації, якщо потрібно
         }
    }
     if (attempts >= 1000) { // Загальне обмеження
        console.error("Failed to generate board after 1000 total attempts.");
     }
    // Повертаємо чисту дошку (тільки 0 і 1)
...
function canPlaceShip(board, x, y, size, isHorizontal) {
    for (let i = 0; i < size; i++) {
        ...
        // Перевірка виходу за межі ТА перевірка накладання (на 1 або 2)
        if (cX >= 10 || cY >= 10 || board[cY][cX] !== 0) return false;
    }
    return true; // Якщо всі клітинки вільні
}
...
function markSurroundings(board, positions) {
    ...
    positions.forEach(({x, y}) => {
        DIRS.forEach(([dx, dy]) => {
            ...
            // Позначаємо як 2 (заборонена зона), тільки якщо там 0
            if (nX >= 0 && nX < 10 && nY >= 0 && nY < 10 && board[nY][nX] === 0) {
                board[nY][nX] = 2; // Позначаємо зону навколо корабля
            }
        });
    });
}
